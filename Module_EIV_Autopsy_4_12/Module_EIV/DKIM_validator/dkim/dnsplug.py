...
def dns_provider():
    try:
        #data_localization=os.getenv('APPDATA').replace("\\","/")+"/.autopsy/dev/python_modules/Module_EIV/DKIM_validator/data"
        data_localization=str(Path(__file__).resolve().parent.parent.joinpath("data")).replace("\\","/")
        file = open(data_localization+"/nameservers.csv","r+")
        if os.stat(data_localization+"/nameservers.csv").st_size==0:
